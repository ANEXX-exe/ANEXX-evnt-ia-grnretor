# -*- coding: utf-8 -*-
import os, json, random, asyncio, openai
import discord
...
async def generate_questions(category, difficulty, number=5):
    ...
    try: questions = json.loads(content)
        ...
        # fallback إذا فشل parsing
        questions = [{"نص":"خطأ في توليد الأسئلة","خيارات":["خطأ"],"صح":0,"صعوبة":difficulty}]
    return questions
...
@app_commands.choices(فئة=[app_commands.Choice(name=c,value=c) for c in CATEGORIES])
@app_commands.choices(صعوبة=[app_commands.Choice(name=s,value=s) for s in POINTS.keys()])
async def quiz(inter:discord.Interaction, فئة:app_commands.Choice[str], عدد:int=5, صعوبة:app_commands.Choice[str]=app_commands.Choice(name="Easy",value="Easy")):
